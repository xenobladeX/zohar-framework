...
public abstract class AbstractCacheManager
    ...
    public Cache getCache(String name, MultiLayerCacheConfig multiLayerCacheConfig) {
        ...
        synchronized (this.cacheContainer) {
            ...
            // 新建一个Cache对象
            Cache cache = getMissingCache(name, multiLayerCacheConfig);
            if (cache != null) {
                // 装饰Cache对象
     ...
     *
     * @param name                 缓存名称
     * @param multiLayerCacheConfig 缓存配置
     * @return {@link Cache}
     */
    ...
    public List<CacheStatsInfo> listCacheStats(String cacheName) {
        return BeanFactory.getBean(CacheStatsService.class).listCacheStats(cacheName);
    }

    @Override
    public void resetCacheStat() {
        BeanFactory.getBean(CacheStatsService.class).resetCacheStat();
    }
