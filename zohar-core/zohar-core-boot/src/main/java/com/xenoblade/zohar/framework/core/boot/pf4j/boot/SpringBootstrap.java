 ...
 * @since 1.0.0
 */
@Slf4j public class SpringBootstrap extends SpringApplication {
    ...
    public static final String[] DEFAULT_EXCLUDE_CONFIGURATIONS = {
            "com.xenoblade.zohar.framework.core.starter.config.Pf4jAutoConfiguration",
            // Spring Web MVC
     ...
     */
    @SuppressWarnings("JavadocReference") public SpringBootstrap(SpringBootPlugin plugin,
                                                                 Class<?>... primarySources) {
        super(new DefaultResourceLoader(plugin.getWrapper().getPluginClassLoader()),
                primarySources);
        this.plugin = plugin;
        ...
        Map<String, Object> presetProperties = ((SpringBootPluginManager) plugin.getWrapper()
                .getPluginManager()).getPresetProperties();
        if (presetProperties != null) {
            this.presetProperties.putAll(presetProperties);
        }
        this.presetProperties.put(PROPERTY_NAME_AUTOCONFIGURE_EXCLUDE, getExcludeConfigurations());
    }
    ...
    public ConfigurableApplicationContext createApplicationContext(){
        ...
        AnnotationConfigApplicationContext applicationContext = (AnnotationConfigApplicationContext) BeanUtils
                .instantiateClass(Class.forName(DEFAULT_CONTEXT_CLASS).getConstructor(
...
//        hackBeanFactory(applicationContext);
    ...
    private String getProperties(Environment env, String propName, int index) {
        String prop = env.getProperty(String.format("zohar-plugin.%s[%s]", propName, index));
        if (prop == null) {
            prop = env.getProperty(String.format("zohar-plugin.%s.%s", propName, index));
        }
        if (prop == null) {
            prop = env.getProperty(String.format("zohar-plugin.%s[%s]",
