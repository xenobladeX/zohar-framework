...
import java.util.*;
import org.zohar.framework.core.plugin.api.util.ClassUtils;
...
public class ExtensionAnnotationProcessor extends AbstractProcessor {
    ...
    private Map<String, Set<String>> extensions = new HashMap<>(); // the key is the plugin point
    private Map<String, Set<String>> oldExtensions = new HashMap<>(); // the key is the plugin point
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        // collect nested plugin annotations
        ...
        // process nested plugin annotations
        ...
        // written to prevent classloaders from falling back to plugin descriptor
    ...
    private void processExtensionElement(Element element) {
        ...
        // check if class extends/implements an plugin point
        if (!ignoreExtensionPoint && !isExtension(element.asType())) {
            error(element, "%s is not an plugin (it doesn't implement ExtensionPoint)", element);
            return;
        }
        ...
        if (extensionPointElements.isEmpty()) {
            error(element, "No plugin points found for plugin %s", extensionElement);
            return;
        }
    ...
    private List<TypeElement> findExtensionPoints(TypeElement extensionElement) {
        ...
        // use plugin points, that were explicitly set in the plugin annotation
        ...
        // detect plugin points automatically, if they are not explicitly configured (default behaviour)
        else {
            ...
            if (extensionPointElements.isEmpty() && ignoreExtensionPoint) {
                if (interfaces.isEmpty()) {
                    error(extensionElement, "Cannot use %s as plugin point with %s compiler arg (it doesn't implement any interface)",
                            extensionElement, IGNORE_EXTENSION_POINT);
                } else if (interfaces.size() == 1) {
                    extensionPointElements.add(getElement(interfaces.get(0)));
                } else {
                    error(extensionElement, "Cannot use %s as plugin point with %s compiler arg (it implements multiple interfaces)",
                            extensionElement, IGNORE_EXTENSION_POINT);
                }
            }
        }
