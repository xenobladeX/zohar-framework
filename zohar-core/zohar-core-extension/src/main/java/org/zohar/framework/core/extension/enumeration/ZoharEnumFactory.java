 ...
 */
package org.zohar.framework.core.extension.enumeration;
...
import org.zohar.framework.core.extension.exception.ExtensionRuntimeException;
...
public class ZoharEnumFactory implements EnumFactory {
     ...
     * @return
     */
    private  <T extends Comparable> Boolean add(IEnum<T> e) {
    ...
    @Override
    public Boolean resolve(Class<? extends IEnum> enumClass) {
        // get all enums
        try {
            ...
            if (result != null) {
                ...
                zoharEnums.stream().forEach(zoharEnum -> add(zoharEnum));
            } else {
                log.warn("No enum found in {}, will ignore", enumClass.getName());
                return false;
            }
