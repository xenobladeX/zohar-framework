...
/**
 * BasicAuthenticationFailureHandler
 * @author xenoblade
 * @since 1.0.0
 */
@Setter
...
public class BasicAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler{
    ...
    private LoginExceptionConverter loginExceptionConverter = new DefaultLoginExceptionConverter();
    ...
    @Override public void onAuthenticationFailure(HttpServletRequest request,
        ...
        switch (loginType) {
            case BODY:
            {
                ResponseEntity responseEntity = loginExceptionConverter.convert(exception);
                Method method = ClassUtil
                        .getPublicMethod(loginExceptionConverter.getClass(), "convert", Exception.class);
